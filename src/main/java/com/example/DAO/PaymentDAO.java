...
public class PaymentDAO {
    ...
    public Payment getByName(String paymentName) {
        ...
        // Các DB cũ có thể chưa chạy phần seed Payment. Tự khởi tạo phương thức
        // hợp lệ một lần để checkout không bị chặn chỉ vì thiếu dữ liệu cấu hình.
        if (paymentName != null && ("Tiền mặt".equals(paymentName) || "Chuyển khoản".equals(paymentName))) {
            Payment missing = new Payment();
            missing.setPaymentName(paymentName);
            if (insert(missing)) return getByName(paymentName);
        }
        return null;
    }
