...
<div class="wire-page">
  ...
  <main class="main-content">
    ...
    <!-- CẢM NHẬN / HOẠT ĐỘNG GẦN NHẤT: giữ lại trong session để luôn hiển thị ở trang liên hệ -->
      <div style="margin:28px 0; border:1.5px solid var(--border-soft); border-radius:var(--radius-lg); background:var(--pink-light); padding:22px 26px;">
        <div style="font-size:15px; font-weight:800; text-transform:uppercase; color:var(--text-chocolate); margin-bottom:14px;">CẢM NHẬN KHÁCH HÀNG</div>
        <c:if test="${not empty sessionScope.lastFeedback}">
          <div style="background:#fff; border-radius:var(--radius-md); padding:14px 16px; margin-bottom:10px; font-size:13px; color:var(--text-muted);">
            <strong style="color:var(--pink-primary);">${sessionScope.lastFeedback.name}</strong>
            <span> · ${sessionScope.lastFeedback.topic} · ${sessionScope.lastFeedback.createdAt}</span>
            <div style="margin-top:6px; color:var(--text-chocolate);">“${sessionScope.lastFeedback.content}”</div>
          </div>
        </c:if>
        <c:forEach items="${publicFeedbacks}" var="feedback" varStatus="loop">
          <c:if test="${empty sessionScope.lastFeedback or feedback.userId ne sessionScope.account.userId or loop.index gt 0}">
            <div style="background:#fff; border-radius:var(--radius-md); padding:14px 16px; margin-bottom:10px; font-size:13px; color:var(--text-muted);">
              <strong style="color:var(--pink-primary);"><c:out value="${feedback.name}"/></strong>
              <span> · <c:out value="${feedback.topic}"/> · <fmt:formatDate value="${feedback.createdAt}" pattern="dd/MM/yyyy HH:mm"/></span>
              <div style="margin-top:6px; color:var(--text-chocolate);">“<c:out value="${feedback.content}"/>”</div>
            </div>
          </c:if>
        </c:forEach>
        <c:if test="${not empty sessionScope.lastOrder}">
          <div style="background:#fff; border-radius:var(--radius-md); padding:14px 16px; font-size:13px; color:var(--text-muted);">
            <strong style="color:var(--pink-primary);">Đặt hàng thành công</strong>
            <span> · Mã đơn #BM${sessionScope.lastOrder.orderId}</span><br>
            <span>Ngày đặt: <fmt:formatDate value="${sessionScope.lastOrder.orderDate}" pattern="dd/MM/yyyy HH:mm"/> · </span>
            <strong><fmt:formatNumber value="${sessionScope.lastOrder.finalAmount}" pattern="#,##0"/>đ</strong>
          </div>
        </c:if>
        <c:if test="${empty sessionScope.lastFeedback and empty sessionScope.lastOrder}">
          <div style="background:#fff; border-radius:var(--radius-md); padding:16px; color:var(--text-muted); font-size:13px;">
            Cảm nhận và đơn hàng gần nhất của bạn sẽ được hiển thị tại đây.
          </div>
        </c:if>
      </div>
