      ...
      <div class="row">
        ...
        <div class="col-6">
          <div style="height:380px;border:1px solid #000;margin-bottom:16px;background:#f5f5f5;overflow:hidden;">
            <img id="main-product-img" src="images/91865ec3a35f437be843c7b5b3f0c4e0.jpg" alt="Bánh Red Velvet" style="width:100%;height:100%;object-fit:cover;" onError="this.onerror=null;this.src='images/placeholder.jpg';">
          </div>
          <div class="row">
        ...
        <div class="col-6" style="padding-left:30px;">
          <h1 id="product-title" style="font-size:26px;font-weight:bold;margin:0 0 10px 0;">Bánh Red Velvet</h1>
          ...
          <div id="product-price" style="font-size:26px;font-weight:bold;margin-bottom:18px;">180.000đ</div>
          <div style="font-size:14px;color:#444;line-height:1.8;margin-bottom:24px;">
            Bánh Red Velvet nổi bật với cốt bánh velvet cốt mềm mịn, sắc đỏ quyến rũ kết hợp hài hòa cùng lớp cream cheese béo ngậy. Món bánh ngọt hoàn hảo cho mọi buổi tiệc và tiệc kỷ niệm.
          </div>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (btnAddCart) {
    btnAddCart.addEventListener("click", function () {
      var title = document.getElementById("product-title").innerText.trim();
      var price = document.getElementById("product-price").innerText.trim();
      var img = mainImg ? mainImg.src : "images/placeholder.jpg";

      var cart = JSON.parse(localStorage.getItem("bites_cart") || "[]");
      var existingIndex = cart.findIndex(function (item) { return item.name === title; });

      if (existingIndex > -1) {
        cart[existingIndex].quantity = (cart[existingIndex].quantity || 1) + qty;
      } else {
        cart.push({ name: title, price: price, img: img, quantity: qty });
      }
  ...
  function checkFavState() {
    ...
    var isFav = favorites.some(function (f) { return f.name === title; });
    if (iconFav) {
      iconFav.setAttribute("fill", isFav ? "#000" : "none");
    }
  }
  ...
  if (btnFav) {
    btnFav.addEventListener("click", function () {
      ...
      var favorites = JSON.parse(localStorage.getItem("bites_favorites") || "[]");
      var existingIndex = favorites.findIndex(function (f) { return f.name === title; });

      if (existingIndex > -1) {
        favorites.splice(existingIndex, 1);
      } else {
        favorites.push({ name: title, price: price, img: img });
      }
