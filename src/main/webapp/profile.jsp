...
<%-- Chỉ kiểm tra đăng nhập tài khoản --%>
    ...
    <div class="header-actions">
      ...
      <div class="noti-wrapper">
        <button type="button" class="ic-btn" id="btn-noti-toggle" title="Thông báo" onclick="toggleNotificationDropdown(event)">
          ...
          <span class="badge" id="notiCountBadge" style="display: none;">0</span>
        </button>

        <div id="notificationDropdown" class="noti-dropdown-box">
          <div class="noti-dropdown-header">
            <strong>🔔 Thông báo đơn hàng</strong>
            <span class="noti-clear-btn" onclick="markAllNotificationsAsRead()">Đã đọc tất cả</span>
          </div>

          <div class="noti-list-scroll" id="notificationListBody">
            <div style="padding: 24px; text-align: center; color: var(--text-muted); font-size: 12px;">
              Đang tải thông báo...
            </div>
          </div>

          <div style="padding: 10px; text-align: center; border-top: 1px solid var(--border-soft); background: #fafafa;">
            <a href="javascript:void(0)" onclick="openMyOrdersTab()" style="font-size: 11px; font-weight: 800; color: var(--pink-primary); text-decoration: none; text-transform: uppercase;">
              Xem tất cả đơn hàng &rsaquo;
            </a>
          </div>
        </div>
      </div>
    ...
    <form action="update-profile" method="POST" id="profile-form">
      ...
      <div class="profile-grid">
        ...
        <aside class="user-sidebar">
          ...
          <div class="side-nav">
            <div class="side-link active" id="btn-side-info" onclick="switchProfileTab('tab-profile-info', this)">
            ...
            <div class="side-link" id="btn-side-orders" onclick="switchProfileTab('tab-order-history', this)">
  ...
  fetch('api/notifications')
    ...
    .then(function(list) {
      ...
      if (!list || list.length === 0) {
        listBody.innerHTML = '<div style="padding:24px; text-align:center; color:var(--text-muted); font-size:12px;">Bạn chưa có thông báo nào</div>';
        if (badge) badge.style.display = "none";
        return;
      }

      var unreadCount = list.filter(function(n) { return !n.isRead; }).length;
      if (badge) {
        if (unreadCount > 0) {
      ...
      list.forEach(function(n) {
        var bg = !n.isRead ? "background: #fff8f9;" : "background: #ffffff;";
        var icon = n.icon ? n.icon : '📦';
        html += '<a href="javascript:void(0)" class="noti-item-link" onclick="handleNotificationClickItem(' + n.orderId + ')" style="' + bg + '">'
              + '<div style="display:flex; gap:10px; align-items:flex-start;">'
              + '<span style="font-size:18px; line-height:1;">' + icon + '</span>'
              + '<div style="flex:1;">'
              + '<div style="font-size:12.5px; font-weight:700; color:var(--text-chocolate);">' + n.title + '</div>'
              + '<div style="font-size:11.5px; color:var(--text-muted); margin-top:2px; line-height:1.4;">' + n.message + '</div>'
              + '</div></div></a>';
      });
      listBody.innerHTML = html;
    })
    .catch(function(err) {
      console.log("Noti fallback:", err);
    });
}

function handleNotificationClickItem(orderId) {
  var dropdown = document.getElementById("notificationDropdown");
...
function openMyOrdersTab() {
  var orderTabBtn = document.getElementById("btn-side-orders");
  if (orderTabBtn) {
    switchProfileTab('tab-order-history', orderTabBtn);
  }
  var dropdown = document.getElementById("notificationDropdown");
  if (dropdown) dropdown.style.display = "none";
}
...
document.addEventListener("click", function(e) {
  var dropdown = document.getElementById("notificationDropdown");
  var btn = document.getElementById("btn-noti-toggle");
  ...
  fetch('order-detail?orderId=' + orderId)
    ...
    .then(function(data) {
      ...
      data.forEach(function(item) {
        ...
        total += subtotal;
        var pName = item.productName || 'Bánh tươi';
        tbody.innerHTML += '<tr style="border-bottom:1px solid #eee;">'
                         + '<td style="padding:10px; font-weight:600;">' + pName + '</td>'
...
document.addEventListener("DOMContentLoaded", function () {
  loadRealNotifications();
  setInterval(loadRealNotifications, 15000);
